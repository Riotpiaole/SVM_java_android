...
public class TrainingActivity extends Activity {
    ...
    {
        super.onResume();
        final ArrayList< Mat > training_data = new ArrayList<Mat >();
        final SVMClassifier svmC = new SVMClassifier();
                            ...
                            if (person.isDirectory()){
                                ...
                                for (File file : files) {
                                    if (FileHelper.isFileAnImage(file)){
                                        ...
                                        MatName m = new MatName("processedImage" + Integer.toString(outCounter) + "_" + Integer.toString(counter), processedImage);
                                        fileHelper.saveMatToImage(m, FileHelper.DATA_PATH);

                                        //loading datasets

                                        training_data.add( processedImage.reshape(1, 1) );
